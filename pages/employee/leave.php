...
<script>
	$(function(){ 
		$("[name=AddCollector]").click(function(){
			...
			data	:	{action : "Employee_Add", page : 1, Table : 'leave'},
	...
	function showPage(page){
		var serialize = $("[name=search_form_parameter]").serialize()+"&action=emeployee_pagination&page="+page+"&Table=leave";
		$.ajax({
			type	:	"post",
			url		:	"pages/hr/ajax/proccess.php",
			dataType:	"json",
    ...
    function doSave()
    {
        if(confirm("Are you sure want to save?")==true){
			...
			// dataType:	"json",
			// data	:	{action : "Pagination", page : page, Table : 'employee'},
			data	:	$("[name=AddEmployee]").serialize()+"&action=Employee_Save&Table=leave",
			success	:	function(data){
                $("#MyModal").modal('hide');
                showPage(1);
			}
