<div id="wrapper">
	<div id="page-wrapper">
        <div id="listingmediaserver">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">DATA STORAGE OF SUPPLIERS</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
				...
				<!-- div class="panel panel-primary">
					<div class="panel-heading">Parameter</div>
						...
						<div class="panel-body">
							...
							<div class="col-sm-6">
									<div class="form-group" style="margin-right:10px;">
										<label class="control-label">dsos : </label>
										<input name="dsos" class="form-control input-sm" placeholder="[123]" value ="" />
									</div>
...
<script>
	$(function(){ 
		$("[name=AddCollector]").click(function(){
			...
			data	:	{action : "Add", page : 1, Table : 'dsos'},
			success	:	function(data){
                var ButtonList = '<button type="button" class="btn btn-info" onclick="doSave();">Save</button>'+
				    ...
				    $("#MyModal .modal-title").html("Add dsos");
                    ...
                    $('.datepicker').datepicker({
			            ...
			            changeYear: true,
                        dateFormat: 'yy-mm-dd',
                        yearRange: "1000:<?=date("Y",strtotime("+5 years"));?>"
	...
	function showPage(page){
		var serialize = $("[name=search_form_parameter]").serialize()+"&action=Pagination&page="+page+"&Table=dsos";
		$.ajax({
			type	:	"post",
			url		:	"pages/hr/ajax/proccess.php",
			dataType:	"json",
    ...
    function doSave()
    {
        if(confirm("Are you sure want to save?")==true){
			...
			// dataType:	"json",
			// data	:	{action : "Pagination", page : page, Table : 'employee'},
			data	:	$("[name=AddEmployee]").serialize()+"&action=Save&Table=dsos",
			success	:	function(data){
                $("#MyModal").modal('hide');
                showPage(1);
			}
    ...
    function doUpdate(id)
    {
        if(confirm("Are you sure want to update?")==true){
			...
			//dataType:	"json",
			//data	:	{action : "Pagination", page : page, Table : 'employee'},
			data	:	$("[name=AddEmployee]").serialize()+"&action=Update&Table=dsos&id="+id,
			success	:	function(data){
				 $("#MyModal").modal('hide');
                 showPage(1);
			} 
