<div id="wrapper">
	<div id="page-wrapper">
        <div id="listingmediaserver">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">promob1t1</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
				...
				<!-- div class="panel panel-primary">
					<div class="panel-heading">Parameter</div>
						...
						<div class="panel-body">
							...
							<div class="col-sm-6">
									<div class="form-group" style="margin-right:10px;">
										<label class="control-label">promob1t1 : </label>
										<input name="promob1t1" class="form-control input-sm" placeholder="[123]" value ="" />
									</div>
...
<script>
	$(function(){ 
		$("[name=AddCollector]").click(function(){
			...
			data	:	{action : "Add", page : 1, Table : 'promob1t1'},
			success	:	function(data){
                var ButtonList = '<button type="button" class="btn btn-info" onclick="doSave();">Save</button>'+
				    ...
				    $("#MyModal .modal-title").html("Add promob1t1");
	...
	function dynamic_add()
	{
		...
		var html  ='<div class="form-group dynamic_add"><select name = "product[]" class = "form-control "> '+$(".dynamic_add").find("select").html();+'</select></div>';
		var html1  ='<div class="form-group dynamic_add_price"><input type="text" name="price[]" class="form-control " value=""></div>';
		$(".append").append(html);
		$(".append_price").append(html1);
	...
	function Edit(id){
		$.ajax({
			...
			url		:	"pages/hr/ajax/proccess.php",
			//data	:	{action : "DemandLetterTypeForm", DLID : ID},
			data	:	{action : "Edit", page : 1, Table : 'promob1t1',id:id},
			success	:	function(data){
                var ButtonList = '<button type="button" class="btn btn-info" onclick="doUpdate('+id+');">Save</button>'+
								 '<button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>';
				    $("#MyModal .modal-dialog").removeClass("modal-lg").removeClass("modal-sm").addClass("modal-lg");
				    $("#MyModal .modal-title").html("Edit promob1t1");
				    $("#MyModal .modal-body").html(data);
				    $("#MyModal .modal-footer").html(ButtonList);
				    $("#MyModal").modal('show');
	...
	function showPage(page){
		var serialize = $("[name=search_form_parameter]").serialize()+"&action=Pagination&page="+page+"&Table=promob1t1";
		$.ajax({
			type	:	"post",
			url		:	"pages/hr/ajax/proccess.php",
			dataType:	"json",
    ...
    function doSave()
    {
        if(confirm("Are you sure want to save?")==true){
			...
			// dataType:	"json",
			// data	:	{action : "Pagination", page : page, Table : 'employee'},
			data	:	$("[name=AddEmployee]").serialize()+"&action=Save&Table=promob1t1",
			success	:	function(data){
                $("#MyModal").modal('hide');
                showPage(1);
			}
	...
	function viewpromob1t1details(id)
	{
		...
		$.ajax({
			...
			success	:	function(data){
                var ButtonList = '<button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>';
				    $("#MyModal .modal-dialog").removeClass("modal-lg").removeClass("modal-sm").addClass("modal-lg");
				    $("#MyModal .modal-title").html("View promob1t1 Product Details");
				    $("#MyModal .modal-body").html(data);
				    $("#MyModal .modal-footer").html(ButtonList);
				    $("#MyModal").modal('show');
    ...
    function doUpdate(id)
    {
        if(confirm("Are you sure want to update?")==true){
			...
			//dataType:	"json",
			//data	:	{action : "Pagination", page : page, Table : 'employee'},
			data	:	$("[name=AddEmployee]").serialize()+"&action=Update&Table=promob1t1&id="+id,
			success	:	function(data){
				 $("#MyModal").modal('hide');
                 showPage(1);
			} 
