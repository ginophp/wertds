<div id="wrapper">
	<div id="page-wrapper">
        <div id="listingmediaserver">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header">Motorcycle Masterlist</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
...
<script>
	$(function(){ 
		$("[name=AddCollector]").click(function(){
			...
			data	:	{action : "Add", page : 1, Table : 'motorcycle_master_list'},
			success	:	function(data){
                var ButtonList = '<button type="button" class="btn btn-info" onclick="doSave();">Save</button>'+
				    ...
				    $("#MyModal .modal-title").html("Add Motorcycle");
        ...
        $("[name=UploadEmployees]").click(function(){
			...
			url		:	"pages/hr/ajax/proccess.php",
			//data	:	{action : "DemandLetterTypeForm", DLID : ID},
			data	:	{action : "Upload", page : 1, Table : 'motorcycle_master_list'},
			success	:	function(data){
	...
	function PrintMR(id){
		...
		var url = "pages/supply/pdf/letters/irs/mr.php?id="+id;
	...
	function Edit(id){
		$.ajax({
			...
			url		:	"pages/hr/ajax/proccess.php",
			//data	:	{action : "DemandLetterTypeForm", DLID : ID},
			data	:	{action : "Edit", page : 1, Table : 'motorcycle_master_list',id:id},
			success	:	function(data){
	...
	function showPage(page){
		var serialize = $("[name=search_form_parameter]").serialize()+"&action=Pagination&page="+page+"&Table=motorcycle_master_list";
		$.ajax({
			type	:	"post",
			url		:	"pages/hr/ajax/proccess.php",
			dataType:	"json",
    ...
    function doSave()
    {
        if(confirm("Are you sure want to save?")==true){
			...
			// dataType:	"json",
			// data	:	{action : "Pagination", page : page, Table : 'employee'},
			data	:	$("[name=AddEmployee]").serialize()+"&action=Save&Table=motorcycle_master_list",
			success	:	function(data){
                $("#MyModal").modal('hide');
                showPage(1);
			}
    ...
    function doUpdate(id)
    {
        if(confirm("Are you sure want to update?")==true){
			...
			//dataType:	"json",
			//data	:	{action : "Pagination", page : page, Table : 'employee'},
			data	:	$("[name=AddEmployee]").serialize()+"&action=Update&Table=motorcycle_master_list&id="+id,
			success	:	function(data){
				 $("#MyModal").modal('hide');
                 showPage(1);
			} 
    ...
    function doUpload()
    {
        var files = $("[name=FileToUpload]").prop("files")[0];
			...
			data.append("table", "motorcycle_master_list");
