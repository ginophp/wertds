						...
						<div class="panel-body">
							...
							<div class="col-sm-6">
									<div class="form-group" style="margin-right:10px;">
										...
										<input name="supplier" class="form-control input-sm" placeholder="[123]" value ="" />
									</div>
...
<script>
	$(function(){ 
		$("[name=AddCollector]").click(function(){
			...
			data	:	{action : "Add", page : 1, Table : 'supplier'},
			success	:	function(data){
                var ButtonList = '<button type="button" class="btn btn-info" onclick="doSave();">Save</button>'+
				    ...
				    $("#MyModal .modal-title").html("Add supplier");
	...
	function showPage(page){
		var serialize = $("[name=search_form_parameter]").serialize()+"&action=Pagination&page="+page+"&Table=supplier";
		$.ajax({
			type	:	"post",
			url		:	"pages/hr/ajax/proccess.php",
			dataType:	"json",
    ...
    function doSave()
    {
        if(confirm("Are you sure want to save?")==true){
			...
			// dataType:	"json",
			// data	:	{action : "Pagination", page : page, Table : 'employee'},
			data	:	$("[name=AddEmployee]").serialize()+"&action=Save&Table=supplier",
			success	:	function(data){
                $("#MyModal").modal('hide');
                showPage(1);
			}
    ...
    function doUpdate(id)
    {
        if(confirm("Are you sure want to update?")==true){
			...
			//dataType:	"json",
			//data	:	{action : "Pagination", page : page, Table : 'employee'},
			data	:	$("[name=AddEmployee]").serialize()+"&action=Update&Table=supplier&id="+id,
			success	:	function(data){
				 $("#MyModal").modal('hide');
                 showPage(1);
			} 
