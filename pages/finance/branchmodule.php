...
<script>
	$(function(){ 
		$("[name=AddCollector]").click(function(){
			...
			data	:	{action : "Add", page : 1, Table : 'branch'},
			success	:	function(data){
                var ButtonList = '<button type="button" class="btn btn-info" onclick="doSave();">Save</button>'+
				    ...
				    $("#MyModal .modal-title").html("Add Branch");
	...
	function showPage(page){
		var serialize = $("[name=search_form_parameter]").serialize()+"&action=Pagination&page="+page+"&Table=branch";
		$.ajax({
			type	:	"post",
			url		:	"pages/hr/ajax/proccess.php",
			dataType:	"json",
    ...
    function doSave()
    {
        if(confirm("Are you sure want to save?")==true){
			...
			// dataType:	"json",
			// data	:	{action : "Pagination", page : page, Table : 'employee'},
			data	:	$("[name=AddEmployee]").serialize()+"&action=Save&Table=branch",
			success	:	function(data){
                $("#MyModal").modal('hide');
                showPage(1);
			}
    ...
    function doUpdate(id)
    {
        if(confirm("Are you sure want to update?")==true){
			...
			//dataType:	"json",
			//data	:	{action : "Pagination", page : page, Table : 'employee'},
			data	:	$("[name=AddEmployee]").serialize()+"&action=Update&Table=branch&id="+id,
			success	:	function(data){
				 $("#MyModal").modal('hide');
                 showPage(1);
			} 
